...
export default function GamePlayPage() {
  ...
  const fetchState = useCallback(async () => {
    ...
    try {
      // TODO: Use Craic-specific API
      // For now, use existing poker API
      const res = await fetch(`/api/poker/state?tableId=${gameId}&playerId=${playerId}`);
  ...
  const handleStartGame = async () => {
    ...
    try {
      const res = await fetch('/api/poker/start', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tableId: gameId, playerId }),
      });
