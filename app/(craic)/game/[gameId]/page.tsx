...
export default function GameLobby() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        // const res = await fetch(`/api/craic/game/${gameId}`);
        ...
        setState({
          config: {
            ...
            sybilOptions: {
              ...
              bondMechanic: { enabled: true, amount: 1000 },
              coinbaseVerification: { enabled: true },
            },
            createdAt: Date.now(),
          },
          status: 'waiting',
          players: [
            { address: '0xabc...def', name: 'Player 1', seatIndex: 0 },
            { address: '0x123...456', name: 'Player 2', seatIndex: 2 },
          ],
          loading: false,
        });
      } catch (error) {
  ...
  // Run sybil checks when wallet connects
  useEffect(() => {
    if (!isConnected || !address || !state.config) return;

    const runChecks = async () => {
      setSybilChecks({ checking: true, passed: false, results: {}, failedChecks: [] });

      // TODO: Actually run sybil verification
      // const result = await verifySybilRequirements(address, state.config.sybilOptions);
      ...
      setSybilChecks({
        ...
        passed: true,
        results: {
          bond: { passed: true, balance: BigInt(100000000) },
          coinbase: { passed: true },
        },
        failedChecks: [],
      });
    };
    ...
    runChecks();
  }, [isConnected, address, state.config]);
          ...
          <div className="p-4 space-y-3">
            ...
            {state.config.sybilOptions.bondMechanic.enabled && (
              <div className="flex items-center justify-between">
                ...
                {sybilChecks.checking ? (
                  <Loader2 className="w-4 h-4 animate-spin text-gray-500" />
                ) : sybilChecks.results.bond?.passed ? (
                  <CheckCircle className="w-4 h-4 text-emerald-400" />
                ) : (
                  <XCircle className="w-4 h-4 text-red-400" />
                )}
            ...
            {state.config.sybilOptions.coinbaseVerification.enabled && (
              <div className="flex items-center justify-between">
                ...
                {sybilChecks.checking ? (
                  <Loader2 className="w-4 h-4 animate-spin text-gray-500" />
                ) : sybilChecks.results.coinbase?.passed ? (
                  <CheckCircle className="w-4 h-4 text-emerald-400" />
                ) : (
                  <XCircle className="w-4 h-4 text-red-400" />
                )}
